...
class _DeskBookingScreenState extends State<DeskBookingScreen> {
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Row(children: [
              Expanded(child: ListTile(
                title: const Text('Start'), subtitle: Text(startTime != null ? startTime!.format(context) : 'Select'),
                trailing: const Icon(Icons.access_time), onTap: () async {
                  final t = await showTimePicker(context: ctx, initialTime: TimeOfDay.now());
                  if (t != null) setDialogState(() => startTime = t);
                },
              )),
              Expanded(child: ListTile(
                title: const Text('End'), subtitle: Text(endTime != null ? endTime!.format(context) : 'Select'),
                trailing: const Icon(Icons.access_time), onTap: () async {
                  final t = await showTimePicker(context: ctx, initialTime: TimeOfDay.now().replacing(hour: TimeOfDay.now().hour + 1));
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                await _supabase.from('desk_bookings').upsert(bookingData);
                
                DateTime? bookedUntil;
                if (endTime != null) {
                  bookedUntil = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day, endTime!.hour, endTime!.minute);
                }
                await _supabase.from('desks').update({
                  'status': 'Booked',
                  'booked_by': widget.user['employee_id'],
                  'booked_until': bookedUntil?.toIso8601String(),
                  'booking_notes': notesCtrl.text.trim(),
                }).eq('id', room['id']);
  ...
  Color _statusColor(String? status) {
    switch (status) { case 'Available': return Colors.green; case 'Booked': return Colors.blue; case 'Occupied': return Colors.orange; case 'Maintenance': return Colors.red; default: return Colors.grey; }
  }
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Padding(padding: const EdgeInsets.all(12), child: Row(children: [
          _legendDot(Colors.green, 'Available'), const SizedBox(width: 12), _legendDot(Colors.blue, 'Booked'), const SizedBox(width: 12), _legendDot(Colors.orange, 'Occupied'), const SizedBox(width: 12), _legendDot(Colors.red, 'Maintenance'),
        ])),
        Expanded(child: _loading ? const Center(child: CircularProgressIndicator(color: Color(0xFFCC0000))) : GridView.builder(
          padding: const EdgeInsets.all(12), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 10, mainAxisSpacing: 10, childAspectRatio: 1.1),
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              onTap: isMine ? _cancelBooking : (isAvailable ? () => _showBookingDialog(room) : null),
              child: Container(
                decoration: BoxDecoration(color: isMine ? const Color(0xFF38A169) : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: isMine ? Colors.green : color, width: isMine ? 2 : 1)),
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(isMine ? Icons.check_circle : Icons.meeting_room, color: isMine ? Colors.white : color, size: 32), const SizedBox(height: 6),
  ...
  Widget _legendDot(Color color, String label) {
    return Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 10, height: 10, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 4), Text(label, style: TextStyle(fontSize: 10, color: Colors.grey.shade600))]);
  }
}