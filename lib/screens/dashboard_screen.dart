...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    try {
      ...
      if (response != null) {
        setState(() {
          ...
          _userData.addAll(Map<String, dynamic>.from(response));
        });
      } else {
  ...
  Future<void> _loadProfilePicture() async {
    try {
      ...
      if (userId != null) {
        ...
        if (response != null && response['image_data'] != null) {
          if (mounted) {
            setState(() {
              _profileImageBase64 = response['image_data'].toString();
            });
          }
        }
      }
  ...
  String _formatDate(String? date, [String? time]) {
    ...
    try {
      ...
      if (date == now) {
        return 'Today${time != null ? ' at $time' : ''}';
      }
      final yesterday = DateTime.now().subtract(const Duration(days: 1)).toString().substring(0, 10);
      if (date == yesterday) {
        return 'Yesterday${time != null ? ' at $time' : ''}';
      }
      final parts = date.split('-');
      if (parts.length == 3) {
        const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        final month = int.tryParse(parts[1]) ?? 1;
        return '${months[month - 1]} ${parts[2]}, ${parts[0]}${time != null ? ' at $time' : ''}';
      }
  ...
  Widget build(BuildContext context) {
    ...
    } else if (_userData['first_name'] != null) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Churchgate HRIS'),
        actions: [
          Stack(
            children: [
              IconButton(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: const Color(0xFFD4AF37).withOpacity(0.15), blurRadius: 20, offset: const Offset(0, 8))],
                ),
                child: Row(
                  children: [
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: CircleAvatar(
                          ...
                          backgroundImage: _profileImageBase64 != null ? MemoryImage(_fromBase64(_profileImageBase64!)) : null,
                          child: _profileImageBase64 == null
                              ? Text(initials, style: const TextStyle(fontSize: 28, color: Colors.white, fontWeight: FontWeight.bold))
                              : null,
                        ),
                      ),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(name, style: const TextStyle(color: Color(0xFFF5E6CC), fontSize: 18, fontWeight: FontWeight.bold)),
                          ...
                          Container(
                            ...
                            child: Text(position, style: const TextStyle(color: Color(0xFFD4AF37), fontSize: 12)),
                          ),
                          if (department.isNotEmpty)
                            Padding(
                              padding: const EdgeInsets.only(top: 4),
                              child: Text('$department${region.isNotEmpty ? ' • $region' : ''}',
              ...
              Row(
                children: [
                  ...
                  Text('Today: ${DateTime.now().day} ${_getMonthName(DateTime.now().month)} ${DateTime.now().year}',
                  ...
                  Container(
                    ...
                    child: const Text('🟢 Office Day', style: TextStyle(fontSize: 11, color: Colors.green)),
                  ),
                ],
              ),
                    ...
                    children: _recentActivities.asMap().entries.map((entry) {
                      ...
                      return Column(
                        children: [
                          ...
                          if (index < _recentActivities.length - 1) const Divider(),
                        ],
                      );
                    }).toList(),
                  ),
              ...
              Center(
                child: Container(
                  ...
                  child: Text('ID: $employeeId', style: const TextStyle(color: Color(0xFFD4AF37), fontSize: 12, fontWeight: FontWeight.w500)),
                ),
              ),
              const SizedBox(height: 20),
            ],
              ...
              child: Column(
                ...
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage: _profileImageBase64 != null ? MemoryImage(_fromBase64(_profileImageBase64!)) : null,
                    child: _profileImageBase64 == null
                        ? Text(initials, style: const TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold))
                        : null,
                  ),
                  const SizedBox(height: 10),
                  Text(name, style: const TextStyle(color: Color(0xFFF5E6CC), fontSize: 16, fontWeight: FontWeight.bold)),
                  Text('$role${department.isNotEmpty ? ' • $department' : ''}',
                      style: TextStyle(color: Colors.grey.shade400, fontSize: 11)),
                ],
              ),
            ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(children: [Icon(icon, color: color, size: 22), const Spacer(), Icon(Icons.arrow_forward_ios, size: 12, color: Colors.grey.shade400)]),
            const Spacer(),
  ...
  Widget _drawerItem(IconData icon, String title, VoidCallback onTap, {bool red = false}) {
    return ListTile(
      leading: Icon(icon, color: red ? Colors.red : const Color(0xFFCC0000), size: 22),
      title: Text(title, style: TextStyle(fontSize: 13, color: red ? Colors.red : const Color(0xFF1A1A1A))),
      onTap: onTap,
      dense: true,
    );
  }
